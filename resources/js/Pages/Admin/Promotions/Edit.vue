...
const form = useForm({
    ...
    type: props.promotion.type || "welcome",
    status: props.promotion.status || "active",
    start_at: props.promotion.start_at ? props.promotion.start_at.slice(0, 16) : "",
    end_at: props.promotion.end_at ? props.promotion.end_at.slice(0, 16) : "",
    min_deposit: props.promotion.min_deposit || "",
    max_bonus: props.promotion.max_bonus || "",
    bonus_percent: props.promotion.bonus_percent || "",
    fixed_bonus_amount: props.promotion.fixed_bonus_amount || "",
    wagering_multiplier: props.promotion.wagering_multiplier || "",
    eligible_user_level: props.promotion.eligible_user_level || "",
    claim_limit_per_user: props.promotion.claim_limit_per_user || "",
    total_claim_limit: props.promotion.total_claim_limit || "",
    terms_text: props.promotion.terms_text || "",
});
...
<template>
    <AdminLayout>
        <div class="p-4 sm:p-6 max-w-3xl">
            <div class="mb-6">
                <Link :href="route('admin.promotions.show', promotion.id)" class="text-sm text-purple-500 hover:text-purple-400 mb-2 inline-block">&larr; Back to Promotion</Link>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Promotion</h1>
            </div>
            ...
            <form @submit.prevent="submit" class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input v-model="form.code" type="text" class="w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white text-sm" />
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                    <Link :href="route('admin.promotions.show', promotion.id)" class="px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg text-sm">Cancel</Link>

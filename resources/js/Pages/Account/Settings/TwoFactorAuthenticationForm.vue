<script setup>
	...
	const enableTwoFactorAuthentication = () => {
		enabling.value = true;
		router.post(
			window.route("two-factor.enable"),
			{},
			{
				preserveScroll: true,
				onSuccess: () =>
	...
	const showQrCode = async () => {
		const { data } = await axios.get(window.route("two-factor.qr-code"));
		qrCode.value = data.svg;
	};

	const showSetupKey = async () => {
		const { data } = await axios.get(window.route("two-factor.secret-key"));
		setupKey.value = data.secretKey;
	};

	const showRecoveryCodes = async () => {
		const { data } = await axios.get(
			window.route("two-factor.recovery-codes"),
		);
		recoveryCodes.value = data;
	};

	const confirmTwoFactorAuthentication = () => {
		confirmationForm.post(window.route("two-factor.confirm"), {
	...
	const regenerateRecoveryCodes = async () => {
		await axios.post(window.route("two-factor.recovery-codes"));
		showRecoveryCodes();
	};

	const disableTwoFactorAuthentication = () => {
		disabling.value = true;
		router.delete(window.route("two-factor.disable"), {
			preserveScroll: true,
			onSuccess: () => {
				...
				<div v-else>
					<ConfirmsPassword
						...
						<PrimaryButton
							v-if="confirming"
							type="button"
							class="me-3 uppercase text-xs font-semibold"
							:class="{ 'opacity-25': enabling }"
							:disabled="enabling">
							Confirm
						</PrimaryButton>
